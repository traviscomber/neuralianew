-- ============================================
-- DROP AND RECREATE PURCHASES TABLE
-- This will fix the policy conflict permanently
-- ============================================

-- Step 1: Drop the purchases table completely
DROP TABLE IF EXISTS purchases CASCADE;

-- Step 2: Recreate the purchases table
...
-- Step 4: Create fresh policies with new names
...
CREATE POLICY "analytics_sessions_public_insert" ON analytics_sessions FOR INSERT TO anon, authenticated WITH CHECK (true);
CREATE POLICY "analytics_sessions_public_select" ON analytics_sessions FOR SELECT TO anon, authenticated USING (true);

CREATE POLICY "analytics_events_public_insert" ON analytics_events FOR INSERT TO anon, authenticated WITH CHECK (true);
CREATE POLICY "analytics_events_public_select" ON analytics_events FOR SELECT TO anon, authenticated USING (true);

CREATE POLICY "analytics_page_views_public_insert" ON analytics_page_views FOR INSERT TO anon, authenticated WITH CHECK (true);
CREATE POLICY "analytics_page_views_public_select" ON analytics_page_views FOR SELECT TO anon, authenticated USING (true);

CREATE POLICY "analytics_conversions_public_insert" ON analytics_conversions FOR INSERT TO anon, authenticated WITH CHECK (true);
CREATE POLICY "analytics_conversions_public_select" ON analytics_conversions FOR SELECT TO anon, authenticated USING (true);

CREATE POLICY "analytics_heatmaps_public_insert" ON analytics_heatmaps FOR INSERT TO anon, authenticated WITH CHECK (true);
CREATE POLICY "analytics_heatmaps_public_select" ON analytics_heatmaps FOR SELECT TO anon, authenticated USING (true);
...
BEGIN
    UPDATE analytics_sessions
    SET page_views_count = page_views_count + 1, updated_at = NOW()
    WHERE session_id = NEW.session_id;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER update_session_on_page_view
    ...
    FOR EACH ROW EXECUTE FUNCTION update_page_view_count();
...
BEGIN
    ...
    RAISE NOTICE '- purchases (with fresh policies)';
    RAISE NOTICE '- analytics_sessions';
    RAISE NOTICE '- analytics_events';
    RAISE NOTICE '- analytics_page_views';
    RAISE NOTICE '- analytics_conversions';
    RAISE NOTICE '- analytics_heatmaps';
    RAISE NOTICE '========================================';
END $$;
