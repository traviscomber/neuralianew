-- Comprehensive Verification Script (Corrected)
...
EXCEPTION WHEN OTHERS THEN
    ...
    DELETE FROM public.deployed_agents WHERE user_id = test_user_id;
    DELETE FROM public.ai_agents WHERE name = 'Workflow Test Agent';
END $$;

-- 10. Test all neural agent types
SELECT '10. TESTING ALL NEURAL AGENT TYPES' as check_type;

DO $$
DECLARE
    neural_types TEXT[] := ARRAY['neural-executive', 'neural-sales', 'neural-hr', 'neural-support', 'neural-analyst'];
    neural_type TEXT;
BEGIN
    FOREACH neural_type IN ARRAY neural_types
    LOOP
        BEGIN
            INSERT INTO public.ai_agents (
                name, type, description, status, user_id, created_at, updated_at
            ...
            RAISE NOTICE 'SUCCESS: % type works!', neural_type;
            
            -- Clean up
            DELETE FROM public.ai_agents WHERE name = 'Test ' || neural_type;
            
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'ERROR: % type failed: %', neural_type, SQLERRM;
        END;
    END LOOP;
END $$;

-- 11. Final status summary
SELECT '11. VERIFICATION SUMMARY' as check_type;
