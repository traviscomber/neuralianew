...
function projectsMetadata(locale: Locale): Metadata {
  const isES = locale === 'es'
  const canonicalPath = isES ? '/es/proyectos' : '/en/projects'
  const metadata = buildLocalizedMetadata({
    locale,
    path: isES ? '/proyectos' : '/projects',
    title: isES ? 'Proyectos de IA y software | N3uralia' : 'AI and software projects | N3uralia',
    description: isES
...
export default function Page({ params }: PageProps) {
  const locale: Locale = isValidLocale(params.locale) ? params.locale : DEFAULT_LOCALE
  if (locale === 'en') permanentRedirect('/en/projects')
