...
export default async function TrustPage(props: PageProps) {
  ...
  return <main className="retro-page min-h-screen pt-20">
    ...
    <section className="border-b border-[rgba(118,214,214,.16)] py-20"><div className="retro-shell"><div className="mb-12 grid gap-4 lg:grid-cols-[.65fr_1.35fr] lg:items-end"><small>01 / CONTROL</small><h2 className="text-[clamp(34px,4vw,56px)]">{page.principlesTitle}</h2></div><div className="grid gap-px bg-[rgba(118,214,214,.16)] md:grid-cols-2 lg:grid-cols-3">{page.principles.map(([title, body, Icon], i) => <article key={title} className="min-h-[260px] bg-[var(--n3-dark-surface)] p-7"><div className="mb-10 flex items-center justify-between"><Icon className="h-7 w-7 text-[var(--n3-teal-soft)]"/><span className="telemetry">0{i+1}</span></div><h3 className="text-[22px] text-[var(--n3-text-light)]">{title}</h3><p className="mt-5 text-[13px] text-[var(--n3-text-muted)]">{body}</p></article>)}</div></div></section>
    <section className="border-b border-[rgba(118,214,214,.16)] py-20"><div className="retro-shell grid gap-12 lg:grid-cols-[.75fr_1.25fr]"><div><small>02 / CLAIMS</small><h2 className="mt-5 text-[clamp(34px,4vw,56px)]">{page.claimsTitle}</h2><p className="mt-6 max-w-xl text-[14px] text-[var(--n3-text-muted)]">{page.claimsBody}</p></div><div className="border-t border-[rgba(118,214,214,.16)]">{page.claims.map(item => <div key={item} className="flex gap-4 border-b border-[rgba(118,214,214,.16)] py-6"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-[var(--n3-teal-soft)]"/><p className="text-[14px] text-[var(--n3-text-muted)]">{item}</p></div>)}</div></div></section>
    <section className="border-b border-[rgba(118,214,214,.16)] py-20"><div className="retro-shell grid gap-12 lg:grid-cols-[.65fr_1.35fr]"><div><small>03 / EVIDENCE</small><h2 className="mt-5 text-[clamp(34px,4vw,56px)]">{page.evidenceTitle}</h2></div><div className="grid gap-px bg-[rgba(118,214,214,.16)] md:grid-cols-2">{page.evidence.map(item => <div key={item} className="bg-[var(--n3-dark-surface)] p-6"><p className="text-[14px] text-[var(--n3-text-muted)]">{item}</p></div>)}</div></div></section>
    <section className="py-24"><div className="retro-shell relative border-y border-[rgba(118,214,214,.2)] py-16"><small>04 / PROOF</small><div className="mt-5 grid gap-10 lg:grid-cols-[1fr_.8fr] lg:items-end"><div><h2 className="max-w-4xl text-[clamp(36px,4.5vw,62px)]">{page.proofTitle}</h2><p className="mt-6 max-w-2xl text-[14px] text-[var(--n3-text-muted)]">{page.proofBody}</p></div><div className="button-row lg:justify-end"><Link href={projectsPath(locale)} className="retro-button retro-button-primary gap-2">{page.projects}<ArrowRight className="h-4 w-4"/></Link><Link href={`/${locale}/contact`} className="retro-button">{page.contact}</Link></div></div></div></section>
  </main>
}
