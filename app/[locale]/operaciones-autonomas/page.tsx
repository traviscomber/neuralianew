        ...
        <section className="border-b border-border px-4 py-24">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <h1 className="mb-6 text-5xl font-bold text-foreground sm:text-6xl">
              {es ? 'Procesos que continúan sin perder control' : 'Processes that keep running without losing control'}
            </h1>
            <p className="mx-auto mb-8 max-w-3xl text-xl text-muted-foreground">
              {es
                ? 'N3uralia automatiza trabajo repetitivo y flujos de decisión con reglas, datos canónicos, permisos, observabilidad y escalamiento humano.'
                : 'N3uralia automates repetitive work and decision workflows with rules, canonical data, permissions, observability and human escalation.'}
            </p>
            <div className="flex flex-col justify-center gap-4 sm:flex-row">
              <Link href={contact} className="inline-flex items-center justify-center gap-2 bg-primary px-8 py-3 font-semibold text-primary-foreground">
                {es ? 'Evaluar un proceso' : 'Evaluate a process'} <ArrowRight className="h-4 w-4" />
              </Link>
              <Link href={trust} className="inline-flex items-center justify-center gap-2 border border-primary px-8 py-3 font-semibold text-primary">
                {es ? 'Ver confianza y seguridad' : 'See trust & security'} <ShieldCheck className="h-4 w-4" />
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="border-b border-border bg-muted/30 px-4 py-24">
          <div className="mx-auto max-w-4xl">
            ...
            <p className="mb-12 text-lg text-muted-foreground">
              {es ? 'No significa una caja negra sin responsables. Significa reducir intervención manual donde el riesgo y la evidencia permiten hacerlo.' : 'It does not mean a black box with no accountable owner. It means reducing manual intervention where risk and evidence allow it.'}
            </p>
            <div className="grid gap-8">
              {capabilities.map(([title, desc], index) => {
                const Icon = [Brain, Zap, BarChart3, CheckCircle2][index]
                return <div key={title} className="border border-border bg-card p-8"><div className="flex gap-4"><Icon className="h-8 w-8 shrink-0 text-primary"/><div><h3 className="mb-2 text-lg font-bold text-foreground">{title}</h3><p className="text-muted-foreground">{desc}</p></div></div></div>
              })}
            </div>
          </div>
        </section>
        ...
        <section className="border-b border-border px-4 py-24">
          <div className="mx-auto max-w-4xl">
            ...
            <div className="space-y-4">
              {useCases.map((item) => <div key={item} className="flex gap-3 border border-border bg-card p-4"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-primary"/><span>{item}</span></div>)}
            </div>
          </div>
        </section>
        ...
        <section className="px-4 py-24">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mb-8 text-lg text-muted-foreground">
              {es ? 'Definimos baseline, riesgos, puntos de aprobación y métricas antes de presentar ahorro, disponibilidad o autonomía como resultado.' : 'We define the baseline, risks, approval points and metrics before presenting savings, availability or autonomy as an outcome.'}
            </p>
            <Link href={contact} className="inline-flex items-center justify-center gap-2 bg-primary px-8 py-3 font-semibold text-primary-foreground">
              {es ? 'Agendar diagnóstico' : 'Book a diagnosis'} <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        </section>
      </main>
