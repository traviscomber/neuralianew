...
export async function middleware(request: NextRequest) {
  ...
  if (host === "n3uralia.com") {
    ...
    if (pathname === "/") {
      redirectUrl.pathname = `/${getPreferredLocale(request.headers.get("accept-language"))}/`
  ...
  if (!pathname.startsWith("/api")) {
    ...
    if (!locale) {
      if (pathname === "/") {
        request.nextUrl.pathname = `/${getPreferredLocale(request.headers.get("accept-language"))}/`
