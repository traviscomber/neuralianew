...
const copy = {
  en: {
    eyebrow: 'Vision · Recognition · Workflows', title: 'Smart Recognition Systems.', intro: 'Computer vision systems for wildlife, production quality, agriculture and security. We connect detection with operations so you can act with confidence, at scale.',
  ...
  es: {
    ...
    casesEyebrow: '02 · Casos de uso', casesTitle: 'Reconocimiento para operaciones reales.', casesIntro: 'Una capa de inteligencia visual adaptada a cada contexto operacional y conectada al flujo que la rodea.',
...
function FlowIcon({i}:{i:number}){
  const paths=[
    <><rect x="5" y="9" width="17" height="13" rx="1"/><circle cx="13.5" cy="15.5" r="3.5"/><path d="M22 13l5-3v11l-5-3z"/></>,
    <><path d="M4 16s5-8 12-8 12 8 12 8-5 8-12 8S4 16 4 16z"/><circle cx="16" cy="16" r="3"/></>,
    <><circle cx="16" cy="16" r="10"/><path d="M16 9v7l5 3"/></>,
    <><ellipse cx="16" cy="8" rx="8" ry="3"/><path d="M8 8v8c0 2 3.6 3.5 8 3.5s8-1.5 8-3.5V8M8 16v8c0 2 3.6 3.5 8 3.5s8-1.5 8-3.5v-8"/></>,
    <><circle cx="16" cy="16" r="10"/><circle cx="16" cy="16" r="4"/><path d="M16 2v5M30 16h-5M16 30v-5M2 16h5"/></>
  ]
...
export function RecognitionPage({locale}:{locale:Locale}){
  ...
  return <main className={s.page}>
    <section className={s.hero}>
      <div className={`${s.shell} ${s.heroGrid}`}>
        <div className={s.heroCopy}>
          <span className={s.eyebrow}>{t.eyebrow}</span><h1 className={s.title}>{t.title}</h1><p className={s.copy}>{t.intro}</p>
    ...
    <section id="use-cases" className={s.section}><div className={s.shell}>
      ...
      <div className={s.cards}>{t.cases.map((c,i)=><article className={s.card} key={c[0]}><div className={s.cardMedia}><img src={images[i][0]} alt={images[i][1]} className={s.image}/></div><div className={s.cardBody}><span className={s.cardIndex}>0{i+1}</span><h3 className={s.cardTitle}>{c[0]}</h3><p className={s.cardText}>{c[1]}</p></div></article>)}</div>
    </div></section>

    <section className={`${s.section} ${s.value}`}><div className={s.shell}>
    ...
    <section className={`${s.section} ${s.platform}`}><div className={`${s.shell} ${s.platformGrid}`}>
