    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="py-16 px-4 border-b border-border bg-background">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-2xl font-semibold">{isES ? "Conecta capacidades con sistemas reales" : "Connect capabilities to real systems"}</h2>
          <div className="mt-6 flex flex-wrap gap-3">
            <Link href={href(isES ? "/soluciones" : "/solutions")} className="px-5 py-3 border border-border hover:border-primary/50">{isES ? "Soluciones" : "Solutions"}</Link>
            <Link href={href("/platform")} className="px-5 py-3 border border-border hover:border-primary/50">{isES ? "Plataforma" : "Platform"}</Link>
            <Link href={href(isES ? "/proyectos" : "/projects")} className="px-5 py-3 border border-border hover:border-primary/50">{isES ? "Proyectos" : "Projects"}</Link>
            <Link href={href("/agentes-ia-chile")} className="px-5 py-3 border border-border hover:border-primary/50">{isES ? "Agentes de IA en Chile" : "AI agents in Chile"}</Link>
          </div>
        </div>
      </section>
      <section className="py-20 px-4 bg-background"><div className="max-w-2xl mx-auto text-center"><h2 className="text-3xl font-bold mb-4">{isES ? "¿Listo para evaluar un caso?" : "Ready to evaluate a use case?"}</h2><p className="text-lg text-muted-foreground mb-8">{isES ? "Revisamos datos, arquitectura, riesgo y valor antes de definir el sistema." : "We review data, architecture, risk and value before defining the system."}</p><Link href={href("/diagnostico")} className="inline-flex items-center gap-2 px-8 py-3 bg-primary text-primary-foreground rounded-lg font-semibold hover:bg-primary/90 transition-colors">{isES ? "Agendar diagnóstico" : "Book a diagnosis"}<ArrowRight className="w-4 h-4"/></Link></div></section>
    </main>
  )
}
