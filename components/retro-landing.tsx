...
function ProjectsSection({ locale }: { locale: Locale }) {
  return <section className="retro-light" id="projects"><div className="retro-shell project-shell"><header className="light-intro"><small>Our projects</small><h2>Real systems.<br />Real impact.</h2><p>Selected projects show N3uralia&apos;s work converting complexity into measurable operational value.</p></header><div className="project-list">
    {projects.map(([no, name, body, industry, image, anchor]) => <Link key={name} href={`/${locale}/proyectos#${anchor}`} className="project-row"><div className="project-copy"><span>{no} —</span><h3>{name}</h3><p>{body}</p><small>{industry}</small></div><div className="project-image"><Image src={image} alt={`${name} interface`} fill sizes="(min-width: 900px) 52vw, 100vw" className="object-cover object-center" /></div></Link>)}
    <RetroButton href={`/${locale}/proyectos`}>See all projects</RetroButton>
  </div></div></section>
}
